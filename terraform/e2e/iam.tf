...
locals {
  mobile_e2e_assume_role_policy = jsonencode({
    Statement = [{
      Action = "sts:AssumeRoleWithWebIdentity"
      Condition = {
        StringEquals = {
          "token.actions.githubusercontent.com:aud"        = "sts.amazonaws.com"
          "token.actions.githubusercontent.com:repository" = var.mobile_github_repository
          "token.actions.githubusercontent.com:sub"        = "repo:${var.mobile_github_repository}:environment:${var.mobile_github_environment}"
        }
      }
      Effect = "Allow"
      Principal = {
